mtfc <- function(approach, base, nn = NULL, bounds = NULL, ...) {
  ...
  # Set class of 'base' to include 'approach'
...
mtfc.proj <- function(base, n, cov_mat, p, ina, ...) {
  ...
  if (missing(base) | missing(cov_mat)) {
    cli_abort(
      "Mandatory arguments: {.arg base} and {.arg cov_mat}.",
      call = NULL
    )
  }
  ...
  if (isDiagonal(cov_mat)) {
    ...
    basec <- Matrix::tcrossprod(base, cov_inv) %*%
...
mtfc.osqp <- function(
  ...
  if (!is.null(nn) & !is.null(mtfore)) {
    ...
    if (!is.null(bounds)) {
      id_b <- which(apply(mtfore[, bounds[, 1], drop = FALSE], 1, function(x) {
  ...
  if (!is.null(nn)) {
    if (!(nn %in% c("osqp", TRUE, "proj_osqp"))) {
      cli_warn(
        "Non-negative combined forecasts obtained with osqp.",
        call = NULL
      )
    }
  ...
  osqp_step <- apply(base[id, , drop = FALSE], 1, function(x) {
    ...
    # OSQP v1.0
    if (is.null(mtf$info$prim_res)) {
      if (!is.null(mtf$info$pri_res)) {
        mtf$info$prim_res <- mtf$info$pri_res
      } else {
        mtf$info$prim_res <- NA
      }
    }
    ...
    if (mtf$info$status_val != 1) {
      cli_warn(
        c(
          ...
          "i" = "OSQP prim_res = {mtf$info$prim_res}"
        ),
        call = NULL
      )
    }
